...
#define DIV_ZERO "cannot divide by zero ts will break the kernel"
#define RED 0x04
#define GREEN 0x02
#define BLUE  0x01
...
void kernel_main(void)
{
    ...
    kstrcpy(files[0].name ,"hello.txt");
    files[0].start_sector = 5;
    files[0].size = 200;
    ...
    while (running)
    {
        ...
        else if(strcmp(buffer,"chuser") == 0){
        ...
        if (strcmp(buffer, "shutdown")==0){
        ...
        else if (strcmp(buffer, "kernelpanic") ==0) {
            kernel_panic();
        }
        
        else if (strcmp(buffer, "beepon") ==0){
            beep_on(1000);
            pit_wait(50);
            beep_off();
        }

        else if (strcmp(buffer, "truefalse") == 0){
            char argua[128];
            char argub[128];
            ...
            ata_read_sector(f->start_sector, sector);
            ...
            kprint("enter write: ", GREEN);
            kfgets(writing, 128);

            for (int i = 0; i < 128 && writing[i] != '\0'; i++) {
                ((char*)sector)[i] = writing[i];
            }

            ata_write_sector(f->start_sector, sector);

            kprint("written to disk\n", GREEN);
        }
...
void kfgets(char* buffer, int max)
{
    ...
    while (1)
    {
        char c = keyboard_read_char(); // ONLY THIS
